      ...
      <div id="tampilkanTransaksiPeriode">
        <table class="table table-bordered table-hover table-sm" id="tblTransaksi">
          ...
          <tbody>
            ...
            $no = 1;
            $ttl= 0;
            $sql = "SELECT * FROM tbl_transaksi WHERE kode_member = '$kode_member' AND total_bayar > 0 ORDER BY tgl_transaksi DESC, no_transaksi DESC";
                ...
                <td>
                  <table class="table table-bordered table-sm">
                    ...
                    <tbody>
                      <?php
                      $nomer = 1;
                      $sql1 = "SELECT a.id_detail, a.qty, a.harga, a.dikerjakan, b.nama_menu FROM tbl_transaksi_detail a INNER JOIN tbl_menu b ON a.id_menu = b.id_menu WHERE a.no_transaksi = '$no_transaksi'  ORDER BY a.id_detail";
                      $query1 = mysqli_query($koneksi, $sql1);
                      while ($data1 = mysqli_fetch_array($query1)) { 
                        $dikerjakan = $data1['dikerjakan']; ?>
                        <tr>
                          ...
                          <?php 
                          if($dikerjakan==""){?>
                            <td align="center"><a href="transaksi-member-delete.php?id_detail=<?= $data1['id_detail']; ?>" class="badge badge-danger p-2 delete-data" title="Delete"><i class="fas fa-trash"></i></a></td>
                            <?php 
                          }?>
                        </tr>
                      <?php
...
<script>
  $(document).ready(function() {
    $(document).on('click', '#periodeCari', function() {
      ...
      var kode_member   = $('[name="kode_member"]').val();
      $.ajax({
        method: 'POST',
        data: {
